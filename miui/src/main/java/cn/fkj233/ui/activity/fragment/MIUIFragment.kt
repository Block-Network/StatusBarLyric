...
class MIUIFragment constructor(val callBacks: (() -> Unit)? = null) : Fragment() {
    private var itemList: List<Item>? = null

    @SuppressLint("ClickableViewAccessibility")
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View = ScrollView(context).apply {
                                            ...
                                            when (list) {
                                                is TextV -> {
                                                    ...
                                                    list.onClickListener?.let { unit ->
                                                        setOnClickListener {
                                                            unit()
                                                            this@MIUIFragment.callBacks?.let { it1 -> it1() }
                                                        }
                                                    }
                                                }
                                                ...
                                                is TextWithSpinnerV -> {
                                                    ...
                                                    setOnTouchListener { view, motionEvent ->
                                                        if (motionEvent.action == MotionEvent.ACTION_UP) {
                                                            val popup =  MIUIPopup(context, view, list.spinner.currentValue, {
